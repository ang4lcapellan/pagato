...
export function CategoryStatusDialog({ category, onClose, onSaved }: { category: Category; onClose: () => void; onSaved: (message: string) => void }) {
  ...
  return <Modal title={category.isActive ? tr("Desactivar categoría") : tr("Reactivar categoría")} onClose={onClose} busy={pending}>
    <p className="break-words text-lg font-semibold">{category.name}</p>
    <p className="mt-3 text-sm leading-6 text-[var(--muted)]">{category.isActive ? tr("Dejará de estar disponible para nuevos movimientos y presupuestos. Los registros existentes se conservan. Podrás reactivarla cuando quieras.") : tr("Volverá a estar disponible para clasificar nuevos movimientos. No se creará otra categoría ni se modificará tu historial.")}</p>
    {state.status === "error" && <p role="alert" className="account-alert mt-5">{tr(state.message)}</p>}
    <form action={action}>
      ...
      <div className="modal-actions"><button className="button button-secondary" type="button" disabled={pending} data-modal-close>{tr("Cancelar")}</button><button className="button button-primary" disabled={pending}>{pending ? tr("Actualizando…") : category.isActive ? tr("Confirmar desactivación") : tr("Confirmar reactivación")}</button></div>
    </form>
  </Modal>;
}
