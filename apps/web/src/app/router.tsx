...
function ProtectedRoute() { const {user,loading}=useAuth(); const location=useLocation(); if(loading)return <main className="grid min-h-screen place-items-center bg-bg"><p className="text-text-secondary">Restaurando sesión…</p></main>; return user?<AppShell/>:<Navigate to="/login" replace state={{from:location.pathname}}/>; }
...
export const router=createBrowserRouter([
  ...
  { path:'/app', element:<ProtectedRoute/>, errorElement:<RouteError/>, children:[
